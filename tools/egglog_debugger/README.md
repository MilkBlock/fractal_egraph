...
绑定到基本类型字段的变量（如 `(Num a)` 里的 `a` 是 `Num` 的 `i64` 字段）也在
同一套映射里：`a` 是字段名，`Num` 项本身是查询节点。插件此前只在 DOT 标签上
应用 `display_names`，公式仍显示 `m.arg_i64_00`；插件 `eggPreviewSource` 现已
对 `ir.math_view` 应用同一套 `upright("…")` 替换（插件提交 `0aea01e`），所以
改名后公式显示 `m + n` 而不是 `m.arg_i64_00`。安装的插件若缺少该修复，公式会
退回带访问器后缀的旧显示。
