...
fn constructed_routes_need_materialized_witnesses() {
    execute(&format!(
        r#"{IR}
(let $ports (PCons (Make "Diff" (PCons (External 0 "Math") (PNil)) "Math") (PNil)))
(let $c (CoarseComb (NoParents) "R" $ports))
...
fn mismatched_parent_templates_do_not_supply_bindings() {
    execute(&format!(r#"{IR}
(let $c (SmoothComb (MoreParents (Basic "A") (NoParents)) "R" (PCons (ParentPort 0 0 "Math") (PNil))))
(let $i (Occurrence 1 $c)) (let $wrong (Occurrence 2 (Basic "B")))
...
fn redundant_support_cannot_use_an_indirectly_removed_instance() {
    let program = format!(
        r#"{IR}
(let $base (Basic "P"))
(let $child (SmoothComb (MoreParents $base (NoParents)) "Q" (PNil)))
