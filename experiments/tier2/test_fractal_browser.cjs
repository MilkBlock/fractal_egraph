...
(async()=>{
 ...
 try{
  ...
  await page.goto(pathToFileURL(path.join(__dirname,'fractal.html')).href);await page.waitForLoadState('networkidle');
  ...
  await page.goto(pathToFileURL(path.join(__dirname,'fractal.html')).href+'#chain_1/4');await page.waitForLoadState('networkidle');
  assert.ok((await page.locator('#tags').textContent()).includes('5923'));
