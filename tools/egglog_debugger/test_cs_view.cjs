...
(async()=>{const b=await chromium.launch({headless:true,executablePath:'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'});try{
 ...
 const root=process.argv[2]||'out/cs-composition-final';const manifest=JSON.parse(fs.readFileSync(root+'/rounds/manifest.json'));
 ...
 const ts=frame.saturated_rule_composition.catalog.catalog.triggers;let index=ts.findIndex(t=>t.binding_origin?.composition?.parts.some(i=>(frame.saturated_rule_composition.cs.units[i].depth||0)>0));if(index<0)index=ts.findIndex(t=>t.origin?.candidate_kind==='CCSS');assert(index>=0);
 await p.selectOption('#native-saturated-rule-composition-instance',String(index));
 await p.selectOption('#native-saturated-rule-composition-diagram','cs');
 await p.waitForSelector('#native-saturated-rule-composition-view[data-ready=true] svg',{timeout:30000});
 assert((await p.locator('#native-saturated-rule-composition-view .node').count())>=3);
 assert((await p.locator('#native-saturated-rule-composition-view').textContent()).includes('CCSS'));
 await p.locator('#native-saturated-rule-composition-view').screenshot({path:root+'/cs-reference.png'});
