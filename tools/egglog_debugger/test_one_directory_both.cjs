...
// layer/Fractal data and the SaturatedRuleComposition data live in the same round snapshots, so typing a
// single directory should never require a second path for SaturatedRuleComposition.
...
(async () => {
  ...
  try {
    ...
    // SaturatedRuleComposition from that same directory.
    ...
    const states = await page.locator('#native-saturated-rule-composition option').count();
    assert.ok(states > 1, `expected shared SaturatedRuleCompositions from the same directory, got ${states} options`);
    assert.match(await page.locator('#native-saturated-rule-composition-status').textContent(), /个 Trigger → \d+ 个共享 SaturatedRuleComposition/);
    assert.equal(await page.locator('#native-saturated-rule-composition-path').inputValue(), '',
                 'SaturatedRuleComposition must come from the loaded directory, not a second catalog path');

    assert.deepEqual(errors, []);
    console.log(`one directory serves Fractal and SaturatedRuleComposition: ${dir}`);
  } finally {
    await browser.close();
  }
