...
for t in r['templates']:
    label=t['kind']+' '+t['rule']
    if t['kind']!='Basic':label+='\n(parent[0].out[0], parent[0].out[1], external[0])' if t['rule']=='R15' else '\n'+t['relative_binding']
...
<h1>Tier-1：共享 rule-comb 模板</h1><p>蓝色框只有规则、父组合与相对端口；没有 Entry 或具体数据 ID。绿色框是独立实例记录，虚线指向共享模板，不是模板的子节点。具体事实与 equality 只在实例及其明确父实例间传播。</p>
