...
`event-driven-hierarchical-rule-composition_v0.2.4.pdf`; v0.1 is preserved separately.
The v0.2.4 paper retains the v0.2.1 visual palette and adds native ripen cost, queue-state, and CS-layer coverage charts; the main Add e-graph now comes from native DOT snapshots generated by `tools/paper_egraph_dot.py`. Other figures remain TikZ so their conceptual overlays stay readable, with blue LHS facts, orange RHS facts, purple coarse
...
The top-N ripen-body SVG exporter is `tools/export_ripen_body_svgs.py`; it consumes a saturated-rule-composition catalog and copies native egglog DOT/SVG artifacts written by ripen plus a manifest.
