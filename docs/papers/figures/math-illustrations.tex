...
also distinguish the roles without color. An e-node may be an RHS in one step and
an LHS in the next.}
\label{fig:legend}
\end{figure}}
...
\begin{scope}[on background layer]
\node[clayer,fit=(l0)(r0),label={[font=\small,text=coarsepurple]above:coarse rule layer $C_0$: one coarse composition}] {};
\node[slayer,fit=(l1)(r1),label={[font=\small,text=smoothteal]below:smooth rule layer $S_0$: one smooth composition}] {};
\end{scope}
\end{tikzpicture}
...
retains the Add row after subsumption but marks it hidden. A saturated body must
