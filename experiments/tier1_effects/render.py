...
cases={'union':('Equality effect 解锁',r['union']['analysis']),
    ...
    for c in data['contexts']:
        ...
        for role,es in groups:
            for j,e in enumerate(es):
                ident=f'e{i}_{role}_{j}';text=effect(e)
...
<h1>Tier-1 e-graph</h1><p>来自原生 egglog 小规模语义实验，尚非 Math 历史。绿色为 ready，橙色为 pending；虚线说明前提或候选替代，点线说明 effect。原组合保留，冗余证书仅针对当前使用。</p>
...
<iframe id="view" title="tier1 graph"></iframe><p>完整图保留所有导出节点与边；超长节点标签仅缩短显示，完整内容保存在 tooltip 和 JSON 中。内部关系表的 Unit 输出不代表 Ctx 被 union。</p>
<script>const c=document.getElementById('case'),m=document.getElementById('mode');function show(){let stem=c.value+m.value;document.getElementById('view').src=stem+'.svg';document.getElementById('svg').href=stem+'.svg';document.getElementById('json').href=c.value+'.native.json'}c.onchange=m.onchange=show;show()</script></html>'''.replace('OPTIONS',options)
