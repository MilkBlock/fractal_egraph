...
cases={'concrete':('R10 提公因子 → R15 乘积求导',r['concrete']['analysis']),
       'union':('组合的外部 equality 支撑',r['union']['analysis']),
       'redundancy':('局部依赖冗余',r['redundancy']),
       'before':('局部支撑补充前',r['dynamic']['before']),
       'after':('局部支撑补充后',r['dynamic']['after'])}
...
for key,(title,data) in cases.items():
    ...
    for c in data['contexts']:
        ...
        lines.append(f'c{i} [label={q(label)},fillcolor="'+('#dcefe5' if a else '#edf0fa')+'"];')
    ...
    for p in data['proposals']:
        ...
        lines.append(f'c{p["old"]} -> c{p["replacement_parent"]} [constraint=false,color="'+('#168454' if ok else '#bd3939')+f'",penwidth=2,style=dashed,label={q(label)}];')
    render(key,lines+['}'])
...
<h1>Tier-1 · Rule-comb 分析图</h1><p>这里显示的是分析 rule comb 的 tier-1 元层图，不是 tier-0 数学表达式图。节点 C0/C1 等表示组合上下文，effect 描述其对 tier-0 的事实要求与产出。当前实例由手工契约构建，尚未自动导入 tier-0 历史。所有 Apply 都表示已见证的组合步骤，不再绘制 pending/ready 执行状态；虚线说明前提或候选替代，点线说明 effect。原组合保留，冗余证书仅针对当前使用。</p>
