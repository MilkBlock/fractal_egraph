...
- 选中 fractal 行时，**编辑器上方会出现第二个标签页**「生成的 Fractal .egg（×d）」并自动切过去，
  里面是只读的生成规则（点「原始 .egg」切回运行的程序，两边内容互不影响：生成的文本不会写进编辑器，
  切换标签也不会动编辑器内容或光标）；没有生成规则的行（application / compose、被拒绝的 fractal
  形状）这个标签自动消失。另外预览上方仍有一栏 **生成的 .egg（Fractal 展开）**，显示追加到源码后的那几行
