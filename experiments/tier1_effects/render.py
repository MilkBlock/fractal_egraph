...
<h1>Tier-1：共享 rule-comb 模板</h1><p>蓝色框只有规则、父组合与相对端口；没有 Entry 或具体数据 ID。Empty 是唯一无数据起点，第一次规则执行也表示为 CoarseComb。绿色框是独立实例记录，虚线指向共享模板，不是模板的子节点。具体事实与 equality 只在实例及其明确父实例间传播。</p>
