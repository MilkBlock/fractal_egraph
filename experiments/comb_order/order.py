...
def capture(profile,manifest,tier1):
    ...
    for r in profile['reads']:
        ...
        rs[r['match']].append({'scope':events[r['match']]['scope'],'op':op,'sorts':sorts,'values':r['row'][:len(sorts)],'producer':producer,'read':r['id']})
